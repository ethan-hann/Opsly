...
**In scope — remove:**
- **`.replit`** — the Replit config file. Its three validation `[workflows]` (`typecheck`, `api-server-tests`, `orval-sync`) are now covered by `.github/workflows/ci.yml` (Plan 02), and its `[deployment]` autoscale block is being replaced by the Plan 06 Docker pipeline. Delete the file. Note in the PR that this removes the Replit autoscale deploy config and that the replacement pipeline is a separate follow-up (Plan 06) — no deploy pipeline is added here.
