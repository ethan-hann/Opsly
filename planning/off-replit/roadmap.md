...
| 03 | [Remove dead Replit code](03-remove-dead-replit-code/plan.md) | Delete `.replit` (its validations will be covered by GitHub Actions after Plan 02 completes), remove the unused `@replit/connectors-sdk` dependency, the `REPL_ID`-gated Replit Vite dev plugins + their catalog entries, the `@replit/*` / `stripe-replit-sync` `minimumReleaseAgeExclude` entries, and `replit.md` / `replit.nix`. | Low | 02 (CI ported) |
