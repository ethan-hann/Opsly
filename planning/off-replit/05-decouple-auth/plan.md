...
- **Rename `@workspace/replit-auth-web`** → a neutral name (e.g. `@workspace/auth-web` — confirm with Ethan). This touches:
  ...
  - All ~9 importers in `artifacts/it-task-manager/` (`App.tsx`, `app-layout.tsx`, `org-guard.tsx`, `invite-page.tsx`, `login.tsx`, `org-settings.tsx`, `org-suspended.tsx`) plus the two `vi.mock("@workspace/replit-auth-web", …)` call sites (`org-guard.suspended.test.tsx`, `export-card.sse.test.tsx`).
  - `artifacts/it-task-manager/package.json` dependency entry (`workspace:*`).
  - Any `tsconfig` path mappings / references to the old package name or dir.
- Rebuild the renamed package's `dist` and re-run `pnpm install` so the workspace link updates.
...
Generated when this plan reaches the top of the queue (after 04 lands), with a fresh grep of `replit_oidc` and `@workspace/replit-auth-web` import sites.
